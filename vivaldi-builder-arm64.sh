...
_create_vivaldi_appimage(){
	if wget --version | head -1 | grep -q ' 1.'; then
		wget -q --no-verbose --show-progress --progress=bar "https://repo.vivaldi.com/snapshot/deb/pool/main/$(curl -Ls https://repo.vivaldi.com/snapshot/deb/pool/main/ | grep -i "$APP-$CHANNEL" | grep arm64 | tail -1 | grep -o -P '(?<=href=").*(?=">vivaldi)')"
	else
		wget "https://repo.vivaldi.com/snapshot/deb/pool/main/$(curl -Ls https://repo.vivaldi.com/snapshot/deb/pool/main/ | grep -i "$APP-$CHANNEL" | grep arm64 | tail -1 | grep -o -P '(?<=href=").*(?=">vivaldi)')"
	fi
	ar x ./*.deb
	tar xf ./data.tar.xz
	mkdir "$APP".AppDir
